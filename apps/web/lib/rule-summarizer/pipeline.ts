...
import { db, rule, ruleStack, ruleThreatMap, stack, threat } from "@/lib/db";
...
/**
 * Summarize all unsummarized security-pattern rules for the given stack slug.
 *
 * Finds rules with slug matching `${stackSlug}-%-security-patterns-v1` where
 * `summaryMdx IS NULL`, generates an LLM summary for each, writes it back to
 * the database, and returns the count of newly summarized rules.
 *
 * Never throws — errors per rule are logged via console.error and that rule is
 * counted as skipped.
 */
export async function summarizeRulesForStack(stackSlug: string): Promise<number> {
  ...
  const rules = await db
    ...
    .where(
      and(
        like(rule.slug, `${stackSlug}-%-security-patterns-v1`),
        isNull(rule.summaryMdx)
      )
    );
  ...
  for (const r of rules) {
    try {
      ...
      if (!stackRow) {
        console.error(`  ✗ ${r.slug}: no stack found, skipping`);
        continue;
      }
      ...
      console.error(`  ✗ ${r.slug}: unexpected error — ${err.message}`);
    }
  }
